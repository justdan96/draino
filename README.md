...
Draino can take care of deleting the PV/PCV associated with the evicted pod. This is interesting especially for pods using storage class having local storage attached to the node. Since the old nodes is not eligible for scheduling, the PVC/PV of the pod must be recycled to ensure that the pods can be schedule on another node.
