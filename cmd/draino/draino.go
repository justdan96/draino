...
func main() {
	...
	apiResources, err := kubernetes.GetAPIResourcesForGVK(cs, options.doNotEvictPodControlledBy)
	if err != nil {
	...
	pf = append(pf, kubernetes.UnprotectedPodFilter(append(systemKnownAnnotations, options.protectedPodAnnotations...)...))
	...
	apiResourcesCordon, err := kubernetes.GetAPIResourcesForGVK(cs, options.doNotCordonPodControlledBy)
	if err != nil {
	...
	podFilterCordon = append(podFilterCordon, kubernetes.UnprotectedPodFilter(options.cordonProtectedPodAnnotations...))
