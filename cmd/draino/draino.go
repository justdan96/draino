...
func main() {
	var (
		...
		doNotCordonPodControlledBy    = app.Flag("do-not-cordon-pod-controlled-by", "Do not evict pods that are controlled by the designated kind, empty VALUE for uncontrolled pods, May be specified multiple times.").PlaceHolder("kind[[.version].group]] examples: StatefulSets StatefulSets.apps StatefulSets.apps.v1").Default("", kubernetes.KindStatefulSet).Strings()
	...
	if len(apiResources) > 0 {
		for _, apiResource := range apiResources {
			if apiResource == nil {
				log.Info("Filtering pod the are uncontrolled for eviction")
			} else {
				log.Info("Filtering pod controlled by apiresource for eviction", zap.Any("apiresource", *apiResource))
			}
		}
	...
	if len(apiResourcesCordon) > 0 {
		for _, apiResource := range apiResourcesCordon {
			if apiResource == nil {
				log.Info("Filtering pod the are uncontrolled for cordon")
			} else {
				log.Info("Filtering pod controlled by apiresource for cordon", zap.Any("apiresource", *apiResource))
			}
		}
