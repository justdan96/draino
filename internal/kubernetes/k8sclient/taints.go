...
const (
	...
	// TaintForceDraining is used to mark nodes that will not go through the normal drain/eviction pipeline.
	// Instead of doing a lot of filtering and simulations, we are directly going into drain phase.
	TaintForceDraining DrainTaintValue = "force-draining"
	// TaintForceDrained is used to show that the force drain was successfully done and the node is ready to be shutdown
	TaintForceDrained DrainTaintValue = "force-drained"
)
