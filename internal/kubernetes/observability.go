...
func (s *DrainoConfigurationObserverImpl) getAnnotationUpdate(node *v1.Node) (string, bool, error) {
	...
	LogForVerboseNode(s.logger, node, "InScope information (2)", zap.Bool("outOfDate", valueDesired != valueOriginal),zap.String("original", valueOriginal), zap.String("desired", valueDesired))
...
func (s *DrainoConfigurationObserverImpl) processQueueForNodeUpdates() {
	for {
		...
		func(obj interface{}) {
			...
			nodeName := obj.(string)
			s.logger.Info("processQueueForNodeUpdates", zap.String("node", nodeName))
