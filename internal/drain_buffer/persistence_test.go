...
func TestConfigMapPersistor(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.Name, func(t *testing.T) {
			...
			persistor := NewConfigMapPersistor[string](wrapper.GetManagerClient(), cmName, cmNS)
			err = persistor.Persist(&tt.Entry)
...
func createTestCM(name, ns, data string) corev1.ConfigMap {
	...
	return corev1.ConfigMap{
		...
		Data:       map[string]string{CMDataKey: string(enc)},
	}
}
