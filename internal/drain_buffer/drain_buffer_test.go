...
func TestDrainBuffer(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.Name, func(t *testing.T) {
			...
			interf, err := NewDrainBuffer(ctx, wrapper.GetManagerClient(), tt.Clock, &logger, cmName, cmNS)
			assert.NoError(t, err, "cannot create drain buffer")
			drainBuffer := interf.(*drainBufferImpl)

			// create new entry
			drainBuffer.Register("foobar", tt.DrainBuffer)
			...
			next := drainBuffer.NextDrain("foobar", tt.DrainBuffer)
			if tt.ShouldFind {
